...
This paper makes that claim precise, and it is worth being precise about what is and is not established. What the model delivers is a \emph{selection principle}: competitive division of labor selects technologies by task-conditional marginal economic value per standardized unit of compute. What the model emphatically does \emph{not} deliver is a license to read survival off realized PnL-per-token ratios. Indeed the equilibrium itself supplies two warnings. First, in equilibrium every active use of compute earns the same marginal PnL per token---the common shadow price of compute---so the realized marginal statistic is cross-sectionally uninformative among survivors; what determines selection is the \emph{counterfactual} comparison of technologies on the same task at the same congestion state. Second, average PnL per token (lifetime PnL over lifetime tokens) is not even an estimate of the marginal object, and under the model's own concave production function the average statistic can rank a strictly inferior technology above a strictly superior one. Section~\ref{sec:measure} develops both points and states the conditions under which an empirical PnL-per-compute statistic is a valid estimate of the survival-relevant quantity.

The paper is organized as follows. Section~\ref{sec:economy} sets out a production economy of prediction tasks supplied by itinerant compute, together with the standing assumptions. Section~\ref{sec:equilibrium} characterizes competitive equilibrium as the equalization of marginal PnL per token, and distinguishes the equilibrium shadow price from the technological productivity that governs selection. Section~\ref{sec:displacement} shows that within any subcontractible task, only the frontier technology is executed. Section~\ref{sec:explicit} solves an explicit equilibrium in closed form. Section~\ref{sec:subdivision} gives a microfounded feasible-set argument for why a profit-maximizing general contractor adopts every economically useful refinement of the task partition. Sections~\ref{sec:extinction} and~\ref{sec:shares} construct an explicit example in which a strictly more capable generalist is driven to zero compute share. Section~\ref{sec:limit} states limiting results in both directions, relating asymptotic execution share to membership of the active productivity frontier, and is explicit about the sense in which these are allocation theorems conditional on technological arrival rather than predictions of arrival. Section~\ref{sec:manager} discusses---as interpretation, not theorem---the surviving role of large models as orchestrators. Section~\ref{sec:measure} concludes with what PnL per token does and does not measure.
...
The converse direction requires strictly more than frontier membership ``somewhere,'' and it is worth recording why the naive converse is false. Membership of the frontier on a \emph{measure-zero} set supports no compute. Membership on a positive-measure set is still insufficient if those tasks are \emph{inactive}---if $\theta_G \le \lambda_t$ there, water-filling \eqref{eq:qB} assigns them nothing. And by Remark~\ref{rem:ties}, frontier membership \emph{with ties} is compatible with an equilibrium selection that gives $G$ zero share everywhere. A correct sufficient condition must therefore bound the frontier margin, the activity margin, and the measure simultaneously.
...
\section{What PnL per token does and does not measure}
\label{sec:measure}
...
\subsection{Average PnL per token can invert the true ranking}

Nor is aggregate PnL divided by aggregate tokens an estimate of the marginal object. On a single task, suppressing $\mu$, the average and marginal products of compute run through the frontier technology are
\begin{equation}
AP(q) = \frac{\phi(\theta q)}{q},
\qquad
MP(q) = \theta\, \phi'(\theta q),
\end{equation}
and concavity gives $AP(q) > MP(q)$ for all $q > 0$, with the wedge growing in $q$. The wedge is not innocuous: it can \emph{reverse rankings}. With $\phi(z) = \log(1+z)$, take a superior technology $A$ with $\theta_A = 2$ run at scale $q_A = 100$, and an inferior technology $B$ with $\theta_B = 1$ run at $q_B = 1$. Then
\begin{equation}
AP_A = \frac{\log 201}{100} \approx 0.053,
\qquad
AP_B = \log 2 \approx 0.693,
\end{equation}
so the average statistic ranks the technology with \emph{half} the productivity an order of magnitude higher, purely because $A$ is deployed deep into its diminishing-returns region. Lifetime PnL over lifetime tokens confounds technology with allocation: congestion and task mix enter the ratio on equal terms with $\theta$.
...
\end{enumerate}
Under these conditions $\Delta\PnL/\Delta T \approx \theta_m(B)\,\phi'(z_B)$, and comparisons across models at matched $(B, z_B)$ recover the ranking of $\theta_m(B)$---the ranking that determines execution. An aggregate league-table statistic---total PnL over total tokens across heterogeneous tasks and deployment scales---satisfies none of the four conditions and may order models arbitrarily badly with respect to survival.
...
The claim this paper establishes is therefore \emph{not} that observed PnL per token determines technological survival. It is:
\begin{center}
\emph{Survival is determined by task-conditional relative marginal economic productivity per standardized unit of compute; and incremental PnL per unit of compute cost, evaluated on comparable tasks at a common congestion state, is an empirical estimate of exactly that productivity.}
\end{center}
This sharper statement is also the more useful one, because it says when a PnL-per-token league table is a valid fitness measure and when it is contaminated---by allocation, by task mix, by congestion, or by non-standardized compute. For technological survival, the economically fundamental question is not \emph{how capable is the model?} but \emph{on what part of an increasingly subdivided production chain does this model remain the highest-value use of compute?}
