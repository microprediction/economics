...
Refinement and technological arrival must not be confounded: from date $t$ to $t{+}1$ both the partition and the model set change, and a value gain may come from either. Write $V^*(\mathcal{P}; \mathcal{M})$ for the optimal value of \eqref{eq:microV} over $\mathcal{P}$-measurable allocations using the models in $\mathcal{M}$, for partitions \emph{adapted} to $\mathcal{M}$: each $A_m$ a union of cells and each $\theta_m$ measurable with respect to the partition. On such a partition an allocation is a finite-dimensional vector $(q_{mB})$ on a compact simplex, so the maximum is attained. The date-to-date comparison then decomposes as
\begin{equation}
...
\label{prop:strict}
Fix $\mathcal{M}$ and let $\mathcal{P}'$ refine $\mathcal{P}$, both adapted to $\mathcal{M}$. Suppose the optimum under $\mathcal{P}'$ assigns strictly positive compute to two children $B_1, B_2 \subset B$ of some cell of $\mathcal{P}$, with distinct unique frontier suppliers $m_1 \neq m_2$ in $\mathcal{M}$. Then $V^*(\mathcal{P}'; \mathcal{M}) > V^*(\mathcal{P}; \mathcal{M})$.
\end{proposition}
