...
(iii) $\phi : [0,\infty) \to [0,\infty)$ is continuously differentiable with $\phi(0) = 0$, $\phi' > 0$, and $\phi'$ strictly decreasing.
...
Refinement and technological arrival must not be confounded: from date $t$ to $t{+}1$ both the partition and the model set change, and a value gain may come from either. Write $V^*(\mathcal{P}; \mathcal{M})$ for the supremum of \eqref{eq:microV} over $\mathcal{P}$-measurable allocations using the models in $\mathcal{M}$. The date-to-date comparison then decomposes as
\begin{equation}
...
\label{prop:strict}
Fix $\mathcal{M}$ and let $\mathcal{P}'$ refine $\mathcal{P}$. Suppose the optimum under $\mathcal{P}'$ assigns strictly positive compute to two children $B_1, B_2 \subset B$ of some cell of $\mathcal{P}$, with distinct unique frontier suppliers $m_1 \neq m_2$ in $\mathcal{M}$. Then $V^*(\mathcal{P}'; \mathcal{M}) > V^*(\mathcal{P}; \mathcal{M})$.
\end{proposition}
...
\right\}
\end{equation}
satisfies $\mu(F_t) \ge \delta$. Then $Q_{G,t}$ is bounded away from zero: there exists $\eta > 0$, depending only on $\varepsilon_r$, $\delta$, $\bar\theta$, and $\phi$, with $Q_{G,t} \ge \eta$ for all $t$.
\end{theorem}

\begin{proof}
On $F_t$ the generalist is the unique frontier technology, so by Proposition~\ref{prop:displacement} it performs all executed work there. The activity condition $\theta_G(x)\,\phi'(0) \ge \lambda_t + \varepsilon_r$ makes the marginal return of the first token strictly exceed the shadow price, so equilibrium compute on such a cell is strictly positive and satisfies $\theta_G(x)\, \phi'\!\big(\theta_G(x)\, q(x)\big) = \lambda_t \le \theta_G(x)\,\phi'(0) - \varepsilon_r$. Since $\phi'$ is continuous and strictly decreasing and $\theta_G \le \bar\theta$, this forces $q(x) \ge q_{\min}(\varepsilon_r, \bar\theta, \phi) > 0$ uniformly on $F_t$. Hence $Q_{G,t} \ge \delta\, q_{\min} = \eta$.
\end{proof}
...
Concavity alone does not deliver the ranking. At the Inada boundary, admitting an infinite right derivative at zero, the power family $\phi(z) = z^\alpha$, $0 < \alpha < 1$, has constant elasticity, so $AP/\lambda = 1/\alpha$ for every $\theta$: competitive PnL per token then carries no ranking information at all. Within the stated smoothness class, take $\phi(z) = z + a(1 - e^{-z})$ with $a > 0$: then $\phi(0) = 0$, $\phi'(z) = 1 + a e^{-z} > 0$, $\phi''(z) = -a e^{-z} < 0$, and its elasticity tends to one as $z \to 0$ and as $z \to \infty$ yet, by strict concavity, lies below one at every positive $z$; it therefore increases somewhere, and on that range competitive average PnL reverses the productivity ranking at a common shadow price. Declining production elasticity is therefore the substantive economic condition under which inframarginal PnL per token reveals productivity, and whether the elasticity of alpha with respect to compute declines with crowding is an empirical question.
