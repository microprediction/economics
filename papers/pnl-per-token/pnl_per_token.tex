...
The margin is the same $\lambda$ for every survivor, but the inframarginal rents are not, and at a common $\lambda$ they are monotone in $\theta$. The conditioning on $\lambda$ is essential: a lifetime quotient that mixes dates or markets with different shadow prices can invert under fully competitive deployment ($\theta_A = 100$ earned where $\lambda = 0.01$ averages $0.01\,g(10^4) \approx 0.09$ per token, while $\theta_B = 2$ earned where $\lambda = 1$ averages $2\log 2 \approx 1.39$). The repair is to deflate: $AP/\lambda = g(\theta/\lambda)$ ranks relative productivity monotonically---raw PnL per token is a nominal return, and the shadow value of compute is its deflator. Note also that when a model serves many tasks its quotient is a token-weighted portfolio of $\lambda\,g(\theta_m(B)/\lambda)$: economic fitness is conditional on both model and task, and a single number summarizes the pieces of the frontier a model occupies rather than an intrinsic scalar quality. The two subsections that follow describe the two further ways the comparison degrades.
...
\begin{center}
\emph{Survival is determined by task-conditional marginal economic productivity per standardized unit of compute. A competitive prediction market pays exactly that margin, so average PnL per token ranks technologies at a common shadow price of compute, and degrades insofar as deployment is administered or scarcity regimes are mixed.}
\end{center}
