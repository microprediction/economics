...
\hypersetup{pdftitle={Division of Labor and the Survival Value of PnL per Token},pdfauthor={Peter Cotton}}
\usepackage{enumitem}
...
\author{Peter Cotton\\ \small Global Strategic Minerals Corporation\\ \small\texttt{peter.cotton@gsmc.ai}}
\date{August 2026}
...
This paper makes that claim precise. The model delivers a \emph{selection principle}: competitive division of labor selects technologies by task-conditional marginal economic value per standardized unit of compute. It does not deliver a license to read survival off realized PnL-per-token ratios. The equilibrium itself disciplines measurement. Marginal PnL per token is not intrinsic to a model: it factorizes into a task-conditional productivity and a market-state factor, and the margin itself is equalized across survivors at the shadow price of compute. What a market-scoring-rule prediction mechanism pays, however, is marginal-contribution accounting, so a participant's lifetime PnL over lifetime tokens is an average of true marginal products. We show that for a sole frontier supplier on one task, evaluated at its competitive scale, task-level average PnL per standardized compute unit equals the shadow price of compute divided by the elasticity of task value with respect to effective compute; an arbitrary model-level lifetime quotient is instead a deployment-weighted average of marginal returns and has no such representation without additional structure. Administered deployment---scale set by fiat rather than by itinerant compute---can rank a strictly inferior technology above a strictly superior one. Section~\ref{sec:measure} develops both points and states the conditions under which an empirical PnL-per-compute statistic is a valid estimate of the survival-relevant quantity.
...
The mechanism has classical ancestry. Smith (1776) tied the division of labour to the extent of the market; Ricardo (1817) showed the absolutely better party does not get all the work; Becker and Murphy (1992) bound specialization by coordination costs; Acemoglu and Autor (2011) model production as a task continuum with technologies assigned by comparative advantage. Hanson (2007) supplies the market scoring rules used in Section~\ref{sec:measure}, and Cotton (2022) the vision formalized here, in which analytical tasks evolve into radically low-cost prediction supply chains.
...
(ii) Each model $m$ has a feasibility set $A_m \subseteq \Omega$, the work it can perform, and a measurable productivity function $\theta_m : A_m \to (0, \bar\theta\,]$, bounded above, assigning to each feasible microtask the \emph{effective predictive effort} contributed per token of $m$; allocations satisfy $q_{mB} = 0$ unless $B \subseteq A_m$. At date $t$, each $A_m$ is a union of cells and each $\theta_m$ is $\mathcal{P}_t$-measurable (constant on cells), so that $\theta_m(B)$ is well defined on feasible cells. For a model already in $\mathcal{M}_t$, $\mathcal{P}_t$-measurability persists under refinement; within-cell variation at later dates enters only through newly arriving models, whose productivities may distinguish cells that earlier partitions did not. Infeasibility ($x \notin A_m$) is conceptually distinct from zero productivity, and we model specialists through $A_m$ rather than through $\theta_m = 0$.
(iii) $\phi : [0,\infty) \to [0,\infty)$ is continuously differentiable with $\phi(0) = 0$, $\phi' > 0$, $\phi'$ strictly decreasing, and $\lim_{z \to \infty} \phi'(z) = 0$.
(iv) $K > 0$, every cell of every partition has positive measure, and every cell is feasible for at least one available model.
\end{assumption}
...
\end{equation}
and the expected trading gain from task $B$ is $\mu(B)\,\phi(z_B)$. The concavity of $\phi$ is economically important: it says that marginal alpha declines with crowding. As more predictive computation attacks the same opportunity, its marginal value falls.
...
\begin{equation}
\label{eq:envelope}
\theta^*(B) = \max_{m \in \mathcal{M}_t :\, B \subseteq A_m} \theta_m(B).
\end{equation}
...
\end{equation}
The right-hand side is continuous and strictly decreasing in $\lambda$ on the relevant range, so $\lambda$ and the aggregate task intensities $q_B$ are unique; the model-level allocation is unique when the frontier model on every active task is unique (Remark~\ref{rem:ties}). Equations \eqref{eq:qB}--\eqref{eq:clearing} constitute an explicit competitive equilibrium for a population of itinerant algorithms; the allocation \eqref{eq:qB} is the familiar water-filling solution. A task is active precisely when $\theta^*(B) > \lambda$.

\begin{remark}[Elastic compute supply]
If instead compute is elastically supplied at dollar cost $c$ per token, set $\lambda = c$. Since $\phi'$ decreases to zero (Assumption~\ref{ass:models}(iii)), the entry condition below has a unique positive solution whenever $c < \theta^*(B)\,\phi'(0)$ and $q_B = 0$ otherwise. Entry then occurs until
\begin{equation}
\theta^*(B)\, \phi'\!\left( \theta^*(B)\, q_B \right) = c,
\end{equation}
which is the marginal entry condition for compute: zero marginal profit for the entering token, not zero inframarginal profit for the technology.
\end{remark}

\section{Why available refinement is adopted}
...
Refinement and technological arrival must not be confounded: from date $t$ to $t{+}1$ both the partition and the model set change, and a value gain may come from either. Write $V^*(\mathcal{P}; \mathcal{M})$ for the maximum of \eqref{eq:microV} over $\mathcal{P}$-measurable allocations using the models in $\mathcal{M}$. This is well defined for any finite partition $\mathcal{P}$: the allocation is a finite-dimensional vector on a compact simplex and the objective is continuous, so the maximum is attained. Adaptation of a partition to $\mathcal{M}$---each $A_m$ a union of cells, each $\theta_m$ measurable with respect to the partition---is not needed for the optimization to be well defined; it is needed only to identify the integral formulation with the cell formulation \eqref{eq:V} and to speak of cellwise frontier suppliers. The date-to-date comparison then decomposes as
\begin{equation}
V^*_{t+1} = V^*(\mathcal{P}_{t+1}; \mathcal{M}_{t+1})
\;\ge\; V^*(\mathcal{P}_t; \mathcal{M}_{t+1})
\;\ge\; V^*(\mathcal{P}_t; \mathcal{M}_t) = V^*_t,
\end{equation}
a refinement term and an arrival term. The results below concern the refinement term alone, with the technology set held fixed.

\begin{lemma}[Refinement monotonicity]
\label{lem:monotone}
For any fixed model set $\mathcal{M}$, if $\mathcal{P}'$ refines $\mathcal{P}$ then $V^*(\mathcal{P}'; \mathcal{M}) \ge V^*(\mathcal{P}; \mathcal{M})$.
\end{lemma}

\begin{proof}
Any $\mathcal{P}$-measurable allocation is $\mathcal{P}'$-measurable, so the feasible set weakly expands and the supremum of \eqref{eq:microV} cannot fall.
\end{proof}

\begin{proposition}[Strict value of refinement]
\label{prop:strict}
Fix $\mathcal{M}$ and let $\mathcal{P}'$ refine $\mathcal{P}$, with $\mathcal{P}'$ adapted to $\mathcal{M}$. Suppose the optimum under $\mathcal{P}'$ assigns strictly positive compute to two children $B_1, B_2 \subset B$ of some cell of $\mathcal{P}$, with distinct unique frontier suppliers $m_1 \neq m_2$ in $\mathcal{M}$. Then $V^*(\mathcal{P}'; \mathcal{M}) > V^*(\mathcal{P}; \mathcal{M})$.
\end{proposition}

\begin{proof}
By Proposition~\ref{prop:displacement} applied cellwise under $\mathcal{P}'$, \emph{every} allocation attaining $V^*(\mathcal{P}'; \mathcal{M})$ runs $m_1$ exclusively on $B_1$ and $m_2$ exclusively on $B_2$, with positive compute on each. A $\mathcal{P}$-measurable allocation assigns each model a constant density across the parent cell $B$, so it would require $q_{m_1} > 0$ on $B_1$ and $q_{m_1} = 0$ on $B_2$ simultaneously. Hence no $\mathcal{P}$-measurable allocation attains the fine optimum, and the inequality is strict.
\end{proof}
...
\begin{center}
\emph{subdivision occurs because it allows finer matching of technologies to task-specific productivity advantage.}
\end{center}
...
\end{equation}
on the tasks where the specialist is cheap; that is, the generalist is more capable everywhere while the specialist creates more value per token at home. Accuracy need not convert linearly into trading value---calibration, latency, and downstream execution all intervene---and \eqref{eq:robust} is robust to any such increasing conversion, provided the cost gap outweighs the capability gap after conversion.
...
\end{equation}
Introduce specialist models $S_k$, each \emph{less capable} everywhere, $b_{S_k} = 0.95 < 0.99$ on every task, but cheap at home: with home tasks
\begin{equation}
I_k = \left( 2^{-k},\, 2^{-k+1} \right],
\end{equation}
the specialist's cost is $c_{S_k} = 1$ on $I_k$ and $c_{S_k} = 10$ elsewhere, so
\begin{equation}
\theta_{S_k}(x) =
\begin{cases}
0.95, & x \in I_k,\\
...
\end{cases}
\end{equation}
Every model is feasible everywhere ($A_m = \Omega$), and the generalist is strictly more accurate than every specialist on every task; yet on each $I_k$,
\begin{equation}
b_G > b_{S_k}
\qquad \text{but} \qquad
\theta_G < \theta_{S_k}:
\end{equation}
higher raw ability, lower economic productivity, exactly the configuration \eqref{eq:robust}. Displacement operates through comparative compute efficiency, not through declared incapacity.

Let the division of labor evolve as follows: at date $t$ the partition distinguishes $I_1, \ldots, I_t$ together with the unresolved residual $R_t = [0, 2^{-t}]$, and specialists $S_1, \ldots, S_t$ are available. Note $\mu(R_t) = 2^{-t}$.

By Proposition~\ref{prop:displacement}, every exposed home task $I_k$ is performed entirely by $S_k$, the unique frontier there ($0.95 > 0.2475$). The generalist is the unique frontier only on the residual $R_t$ ($0.2475 > 0.095$), so its execution domain satisfies
\begin{equation}
...
\begin{equation}
E_t = \left\{ x \in \Omega : \theta_G(x) \ge \max_{m \in \mathcal{M}_t :\, x \in A_m} \theta_m(x) \right\}.
\end{equation}
...
\begin{equation}
E_\infty = \left\{ x \in \Omega : \theta_G(x) \ge \sup_{m \in \mathcal{M}_\infty :\, x \in A_m} \theta_m(x) \right\},
...
\begin{equation}
\sup_{m \in \mathcal{M}_\infty :\, x \in A_m} \theta_m(x) > \theta_G(x) \qquad \text{for a.e. } x,
\end{equation}
...
\label{thm:survival}
Suppose there exist $\varepsilon_\theta > 0$, $\varepsilon_r > 0$, and $\delta > 0$ such that for all $t$ the set
\begin{equation}
F_t = \left\{ x \in \Omega :\;
\theta_G(x) \ge \max_{m \in \mathcal{M}_t \setminus \{G\} :\, x \in A_m} \theta_m(x) + \varepsilon_\theta
\;\text{ and }\;
\theta_G(x)\, \phi'(0) \ge \lambda_t + \varepsilon_r
\right\}
\end{equation}
satisfies $\mu(F_t) \ge \delta$, with the convention $\max_\emptyset = -\infty$ when $G$ is the only feasible model at $x$. Then $Q_{G,t}$ is bounded away from zero: there exists $\eta > 0$, depending only on $\varepsilon_r$, $\delta$, $\bar\theta$, and $\phi$, with $Q_{G,t} \ge \eta$ for all $t$.
\end{theorem}

\begin{proof}
On $F_t$ the generalist is the unique frontier technology, so by Proposition~\ref{prop:displacement} it performs all executed work there. The activity condition $\theta_G(x)\,\phi'(0) \ge \lambda_t + \varepsilon_r$ makes the marginal return of the first token strictly exceed the shadow price, so equilibrium compute on such a cell is strictly positive and satisfies $\theta_G(x)\, \phi'\!\big(\theta_G(x)\, q(x)\big) = \lambda_t \le \theta_G(x)\,\phi'(0) - \varepsilon_r$. Explicitly, continuity of $\phi'$ at zero yields $q_{\min} > 0$ with $\bar\theta\,[\phi'(0) - \phi'(\bar\theta\, q_{\min})] < \varepsilon_r$; if $q(x) < q_{\min}$ then $\theta_G(x)[\phi'(0) - \phi'(\theta_G(x)\,q(x))] \le \bar\theta\,[\phi'(0) - \phi'(\bar\theta\, q_{\min})] < \varepsilon_r$, contradicting the activity margin. Hence $q(x) \ge q_{\min}$ uniformly on $F_t$. Hence $Q_{G,t} \ge \delta\, q_{\min} = \eta$.
\end{proof}
...
\subsection{Market scoring rules pay the margin}

Almost nobody measures models economically. Most public leaderboards primarily rank capability, sometimes alongside price and latency; almost none ranks realized economic value net of compute expenditure. One public attempt at a realized-PnL ranking, \texttt{modelrank.net}, currently divides prediction-market PnL by vendor-metered tokens under randomized routing, with open positions marked to market: read through the present model, a randomized experiment estimating average incremental trading contribution over an assigned distribution of tasks and states, with randomization serving causal identification rather than instantiating the itinerant equilibrium. That quotient began as a heuristic for the survival-relevant statistic; the results of this section sharpen it, saying what it estimates and under which conditions it ranks. Its markets run on a logarithmic automated market maker,\footnote{Implementation details at \texttt{modelrank.net}.} the cost-function form of the market scoring rule, so trades are paid literal score increments and the telescoping identity below applies to it directly; the residual gaps are the token denominator, the routing, and the marking of open positions. The prediction-market setting is nevertheless the right instrument, in its market-scoring-rule implementation: there, each entry's payout is the gain in score over the standing forecast, and payouts telescope.\footnote{Hanson, R. (2007). ``Logarithmic market scoring rules for modular combinatorial information aggregation.'' \emph{Journal of Prediction Markets} 1, 3--15.} Other designs, such as continuous double auctions, need not pay literal score increments, and for them trading PnL is an empirical proxy for the same object. The mechanism implements marginal-contribution payments for procured information value, funded by a principal who values the forecast; the $\phi$ below is market-compensated information output rather than a welfare measure, and the theory needs only these private returns. Prediction-market PnL is therefore marginal-contribution accounting: a token of model $m$ entering task $B$ at congestion $z_B$ earns $\theta_m(B)\,\phi'(z_B)$, exactly the selection object. Telescoping gives additivity of payments; identifying them with the production technology takes one definition. Let $p(z)$ be the standing forecast after effective effort $z$ and \emph{define} $\phi(z) = \E[S(p(z), Y)] - \E[S(p(0), Y)]$, the expected score improvement effort has purchased; the form $\phi(z) = \log(1+z)$ is a convenient parametrization of this map, not a consequence of the scoring rule. With compute arriving in increments, each choosing the highest current return, an increment $dq$ on a task earns $d\Pi = \phi'(z)\,dz = \theta\,\phi'(\theta q)\,dq$, so payments integrate to $\int_0^q \theta\,\phi'(\theta s)\,ds = \phi(\theta q)$ for a sole supplier---the case Proposition~\ref{prop:displacement} delivers on strict frontiers; with interleaved suppliers aggregate payments still telescope but individual attribution is path dependent. A participant's lifetime PnL over lifetime tokens is then an average of true marginal products: the deployment-weighted average of $\theta_m(B)\,\phi'(z)$ over the tasks and states it encountered. Whether an average ranks models depends on who chose the deployment, at what scarcity, and on the shape of $\phi$. For a sole supplier on one task, evaluated at its competitive scale, the average relative to the shadow price is an inverse elasticity.

\begin{lemma}[PnL per token is an inverse elasticity]
\label{lem:ap}
Let $\varepsilon_\phi(z) = z\,\phi'(z)/\phi(z)$ denote the elasticity of task value with respect to effective compute. On any active task with productivity $\theta$ at shadow price $\lambda$,
\begin{equation}
\frac{AP(\theta;\lambda)}{\lambda} = \frac{1}{\varepsilon_\phi(z)},
\qquad
\theta\,\phi'(z) = \lambda,
\end{equation}
and $z(\theta,\lambda)$ is increasing in $\theta$. Hence $AP$ ranks $\theta$ at a common $\lambda$ if and only if $\varepsilon_\phi$ is decreasing on the relevant range.
\end{lemma}

\begin{proof}
$AP = \phi(z)/q = \theta\,\phi(z)/z$ and $\lambda = \theta\,\phi'(z)$ give $AP/\lambda = \phi(z)/(z\,\phi'(z))$, and $z = (\phi')^{-1}(\lambda/\theta)$ increases in $\theta$ since $\phi'$ is decreasing.
\end{proof}

\begin{corollary}[Log production ranks]
\label{cor:log}
For $\phi(z) = \log(1+z)$, $\varepsilon_\phi(z) = z/\big((1+z)\log(1+z)\big)$ is strictly decreasing, since $\log(1+z) < z$; hence $AP$ is strictly increasing in $\theta$, explicitly $AP = \lambda\, g(\theta/\lambda)$ with $g(r) = r\log r/(r-1)$ and $g'(r) = (r - 1 - \log r)/(r-1)^2 > 0$.
\end{corollary}

Concavity alone does not deliver the ranking. At the Inada boundary, admitting an infinite right derivative at zero, the power family $\phi(z) = z^\alpha$, $0 < \alpha < 1$, has constant elasticity, so $AP/\lambda = 1/\alpha$ for every $\theta$: competitive PnL per token then carries no ranking information at all. Within the stated smoothness class, take $\phi(z) = 0.1(1 - e^{-10z}) + (1 - e^{-0.1z})$: then $\phi(0) = 0$, $\phi'(z) = e^{-10z} + 0.1\,e^{-0.1z} > 0$, $\phi''(z) < 0$, and $\phi'(z) \to 0$, satisfying every assumption; yet its elasticity rises from about $0.33$ at $z = 0.4$ to about $0.46$ at $z = 1$, and on that range competitive average PnL reverses the productivity ranking at a common shadow price. Declining production elasticity is therefore the substantive economic condition under which inframarginal PnL per token reveals productivity, and whether the elasticity of alpha with respect to compute declines with crowding is an empirical question.

Under log production the margin is the same $\lambda$ for every survivor, but the inframarginal rents are not, and at a common $\lambda$ they are monotone in $\theta$. The conditioning on $\lambda$ is essential: a lifetime quotient that mixes dates or markets with different shadow prices can invert under fully competitive deployment ($\theta_A = 100$ earned where $\lambda = 0.01$ averages $0.01\,g(10^4) \approx 0.09$ per token, while $\theta_B = 2$ earned where $\lambda = 1$ averages $2\log 2 \approx 1.39$). The repair is to deflate: $AP/\lambda = g(\theta/\lambda)$ ranks relative productivity monotonically---raw PnL per token is a nominal return, and the shadow value of compute is its deflator. Alternatively, when compute is elastically supplied at unit dollar cost (the elastic-supply remark of Section~\ref{sec:explicit}), $\lambda = 1$ across unconstrained competitive environments and $AP(\theta) = g(\theta)$ is directly comparable; deflation is needed only where compute is capacity constrained or its real price moves. Note also that when a model serves many tasks its quotient is a token-weighted portfolio of $\lambda\,g(\theta_m(B)/\lambda)$: economic fitness is conditional on both model and task, and a single number summarizes the pieces of the frontier a model occupies rather than an intrinsic scalar quality. Cross-task comparability further presumes a common, or suitably normalized, response function $\phi$ across tasks: with task-specific $\phi_B$, deflated quotients from different tasks reflect different elasticities as well as different productivities. The two subsections that follow describe the two further ways the comparison degrades.
...
Lemma~\ref{lem:ap} requires the deployment to be the competitive one. When deployment is administered instead---scale chosen by fiat rather than by itinerant compute---the same quotient can reverse rankings even within one contemporaneous environment, because an administered allocation need not satisfy the common-$\lambda$ first-order conditions. On a single task, suppressing $\mu$, the average and marginal products of compute run through a technology deployed at scale $q$ are
\begin{equation}
...
\end{enumerate}
Under these conditions $\Delta\PnL/\Delta T \approx \theta_m(B)\,\phi'(z_B)$, and comparisons across models at matched $(B, z_B)$ recover the ranking of $\theta_m(B)$---the ranking that determines execution. Under competitive deployment the quotient needs only shadow-price conditioning (Lemma~\ref{lem:ap}); the four disciplines above are what restore validity when it is not.

\subsection{The intensive and the extensive margin}

PnL per token is an intensive-margin statistic: it measures value per token on the niches a model occupies, conditional on operating there. Production share also requires the extensive margin, the economic mass of those niches: Theorem~\ref{thm:extinction} runs through $\mu(E_t)$, and a model with $\theta = 100$ on a niche of measure $10^{-9}$ combines spectacular PnL per token with negligible share. Commercial survival requires more still. Under log production an active task, $\theta > \lambda$, yields inframarginal rent per unit of task mass
\begin{equation}
R(\theta, \lambda) = \phi(\theta q) - \lambda q = \log\frac{\theta}{\lambda} - 1 + \frac{\lambda}{\theta},
\qquad
R(\theta, \lambda) = 0 \text{ for } \theta \le \lambda,
\end{equation}
above the shadow opportunity cost of its compute; with compute elastically supplied at unit cash cost (the elastic-supply remark of Section~\ref{sec:explicit}), $\lambda = 1$ and $R(\theta) = \log\theta - 1 + 1/\theta$ is operating rent above actual compute cost. It is total rent integrated over owned tasks, not average PnL per token, that must eventually cover a technology's fixed development and maintenance cost. The chain runs: task productivity, PnL per token, allocation, total rents, commercial survival. This paper establishes the middle of the chain; the final arrow is the entry model of Remark~\ref{rem:conditional}.
...
\begin{center}
\emph{Survival is determined by task-conditional marginal economic productivity per standardized unit of compute. Under a market scoring rule each forecast update is paid its score increment, so for a sole frontier supplier on one task at its competitive scale, task-level average PnL per token equals the shadow price of compute divided by the elasticity of task value with respect to effective compute, and at a common shadow price ranks task-level productivity when that elasticity declines with crowding. An arbitrary lifetime quotient has no such representation, and the statistic degrades insofar as deployment is administered or scarcity regimes are mixed.}
\end{center}
...
Acemoglu, D., and Autor, D. (2011). ``Skills, Tasks and Technologies: Implications for Employment and Earnings.'' In Ashenfelter, O., and Card, D. (Eds.), \emph{Handbook of Labor Economics}, Vol.~4B, Elsevier, 1043--1171.

\bibitem{beckermurphy1992}
Becker, G.~S., and Murphy, K.~M. (1992). ``The Division of Labor, Coordination Costs, and Knowledge.'' \emph{Quarterly Journal of Economics} 107(4), 1137--1160.

\bibitem{cotton2022}
Cotton, P. (2022). \emph{Microprediction: Building an Open AI Network}. MIT Press, Cambridge, MA.

\bibitem{hanson2007}
Hanson, R. (2007). ``Logarithmic Market Scoring Rules for Modular Combinatorial Information Aggregation.'' \emph{Journal of Prediction Markets} 1(1), 3--15.

\bibitem{ricardo1817}
Ricardo, D. (1817). \emph{On the Principles of Political Economy and Taxation}. John Murray, London.

\bibitem{smith1776}
Smith, A. (1776). \emph{An Inquiry into the Nature and Causes of the Wealth of Nations}. W. Strahan and T. Cadell, London.
