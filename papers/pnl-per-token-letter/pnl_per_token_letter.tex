...
At the time of writing, public leaderboards rank ability alone; the sole attempt at an economic ranking, \texttt{modelrank.net}, computes a lifetime quotient from prediction-market participation. The results above say how to read it. The quotient aggregates inframarginal rents that rank productivity at a matched shadow price; it degrades when deployment is administered or eras of different compute scarcity are mixed, and a rolling, $\lambda$-deflated version is the defensible statistic. Nor is there a scalar true $\theta_m$ underneath: a model serving many tasks reports a token-weighted portfolio of $\lambda\, g(\theta_m(B)/\lambda)$, because economic fitness is conditional on both model and task. The statistic is therefore not an estimate of intrinsic model quality, since no such scalar exists; it is an equilibrium-weighted measure of the economic niches a model actually occupies, which is a more useful object than another leaderboard. Capability benchmarks measure $b$ alone, while equilibrium prices $b$ jointly with compute cost and task value through $\theta = v(b)/c$.
